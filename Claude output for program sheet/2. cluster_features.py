...
def save_clusters(clusters: List[Dict], output_path: str, source_data: Dict = None):
    """Save cluster results to a JSON file.

    Carries forward top-level metadata (bounding_box, mass_properties, file, etc.)
    from the source features JSON so downstream steps have access to part geometry.
    """
    output = {'clusters': clusters}
    if source_data:
        passthrough_keys = ['file', 'bounding_box', 'mass_properties',
                            'surface_area', 'topology_counts']
        for key in passthrough_keys:
            if key in source_data:
                output[key] = source_data[key]
